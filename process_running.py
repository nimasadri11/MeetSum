...
def write(file, makevocab=False):
    ...
    with open(file, 'r') as f, open(os.path.join(finish_dir, 'eval.bin') , 'wb') as writer:
        reader = csv.reader(f)
        count = 0
        for row in reader:
            if count == 0:
...
vocab_dir = "./vocab-wiki"
""" 
file = 'cl_news_summary_more.csv'
finish_dir = './data/news/main/'
chunks_dir = './data/news/chunks/'
in_file = './data/news/main/train.bin'
...
"""
in_file = './data/wiki/main/train.bin'
file = 'wikihowAll.csv'
